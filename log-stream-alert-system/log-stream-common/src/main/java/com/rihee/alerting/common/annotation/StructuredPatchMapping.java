...
/**
 * {@code @StructuredPatchMapping}은 {@link org.springframework.web.bind.annotation.PatchMapping}의
 * 대체 어노테이션으로, 구조화된 로깅과 추적을 위해 {@code spanLabel} 속성을 필수로 지정하는 PATCH 요청 매핑 어노테이션입니다.
 *
 * <p>기본적으로 {@link RequestMapping}의 {@code method = RequestMethod.PATCH} 동작을 포함하며,
 ...
 * <h2>사용 예:</h2>
 * <pre>{@code
 * @StructuredPatchMapping(
 *     spanLabel = "patchUser",
 *     value = "/user/update"
 * )
 * }</pre>
 *
 * <p>{@code @PatchMapping}, {@code @RequestMapping(method = RequestMethod.PATCH)}의 사용은 금지되며,
 * 모든 PATCH 요청은 반드시 {@code @StructuredPatchMapping}을 통해 정의되어야 합니다. (Checkstyle 정책 적용)</p>
 *
