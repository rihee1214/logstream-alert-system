...
public class DefaultValidatorSingleMessageTest {
  ...
  void collects_single_message_and_emits_to_pipeline(TestDefaultLogValidatorAdapter adapter) {
    ...
    assertThat(result.context().iterator().next()).isInstanceOf(LogNormalMessage.class);
  }

  @Test
  @DisplayName("단일 정상 메시지를 받아서 해당 메시지를 에러처리 후 다음 파이프 라인으로 넘긴다.")
  void collects_single_message_and_emits_error_to_pipeline(TestDefaultLogValidatorAdapter adapter) {
  ...
  @Test
  @DisplayName("단일 정상 메시지를 받아서 해당 메시지를 검증한 후 다음 파이프 라인으로 넘긴다.")
  void collects_single_error_message_and_emits_error_to_pipeline(TestDefaultLogValidatorAdapter adapter) {
