 ...
 * 해당 타입의 {@link LogValidator} 구현체를 탐색하고, 해당 구현체의 {@code builder()} 메서드를
 * 호출하여 검증기 인스턴스를 생성하는 팩토리 역할을 수행합니다.
 *
 * <p>구체 클래스 탐색은 {@link ValidatorType} 애노테이션을 기반으로 하며,
 * 구현체는 {@link #TARGET_VALIDATOR_PACKAGE} 패키지 내에서 검색됩니다.
 *
 * <p>이 클래스는 런타임에 리플렉션과 {@link ClassGraph}를 사용하여 동적으로 인스턴스를 생성합니다.
 *
 * @see LogValidator
 * @see ValidatorType
 * @see com.rihee.alerting.loggingService.validators.impl
 */
public class LogValidatorSpec implements LogProcessorSpec {

  /**
   * {@link LogValidator} 구현체를 탐색할 대상 패키지 경로.
   *
   ...
   */
  private static final String TARGET_VALIDATOR_PACKAGE
                            = "com.rihee.alerting.loggingService.validators.impl";

  private final Builder<?> builder;
  private final String validatorType;

  /**
   * 주어진 설정 맵을 기반으로 {@link LogValidatorSpec}를 생성합니다.
   *
   * <p>설정에서 {@code validator.type} 키를 읽어 해당 타입에 맞는
   * {@link LogValidator} 구현체를 찾고, 해당 구현체의 빌더를 초기화합니다.
   *
  ...
  /**
   * 지정된 {@code validatorMode} 값에 해당하는 {@link LogValidator} 구현체의
   * 빌더를 탐색하고 반환합니다.
   *
   * <p>탐색 방식:
   * <ol>
   *   <li>{@link #TARGET_VALIDATOR_PACKAGE} 패키지를 스캔</li>
   ...
   * @return 해당 타입의 {@link LogValidator.Builder}
  ...
  private static LogValidator.Builder<?> resolveValidatorBuilder(String validatorMode) {
    try (ScanResult scanResult = new ClassGraph()
        .enableAllInfo()
        .acceptPackages(TARGET_VALIDATOR_PACKAGE)
        .scan()) {
   ...
   *
   * <p>빌더 패턴을 사용하여 {@link LogValidator} 구현체 인스턴스를 생성하며,
