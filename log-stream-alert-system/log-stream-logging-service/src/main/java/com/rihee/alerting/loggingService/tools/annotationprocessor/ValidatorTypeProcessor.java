...
/**
 * {@code ValidatorTypeProcessor}는 {@link ValidatorType} 애너테이션이 부여된 클래스들을 대상으로
 * 컴파일 타임에서의 검증을 수행하는 Annotation Processor입니다.
 *
 * <p>이 클래스는 {@link AbstractTypeProcessor}를 상속받아 공통 로직을 재사용하며,
 * {@link #getTargetAnnotationType()}을 통해 처리 대상 애너테이션을 {@code ValidatorType.class}로 지정합니다.
 * 구체적인 검증 로직은 상위 추상 클래스 내의 {@link ProcessorLogic} 구현체들을 통해 수행됩니다.
 *
 * <h3>기본 제공되는 검증 규칙:</h3>
 * <ul>
 *   <li><b>중복 식별자 검사:</b> 동일한 {@code @ValidatorType.value()} 값을 갖는 클래스가
