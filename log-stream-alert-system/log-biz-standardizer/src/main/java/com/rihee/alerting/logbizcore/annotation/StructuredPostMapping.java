...
/**
 * {@code @StructuredPostMapping}은 {@link org.springframework.web.bind.annotation.PostMapping}의
 * 대체 어노테이션으로, 구조화된 로깅과 추적을 위해 {@code spanLabel} 속성을 필수로 지정하는 POST 요청 매핑 어노테이션입니다.
 *
 * <p>기본적으로 {@link RequestMapping}의 {@code method = RequestMethod.POST} 동작을 포함하며,
 ...
 * }</pre>
 *
 * <p>{@code @PostMapping}, {@code @RequestMapping(method = RequestMethod.POST)}의 사용은 금지되며,
 * 모든 POST 요청은 반드시 {@code @StructuredPostMapping}을 통해 정의되어야 합니다. (Checkstyle 정책 적용)</p>
 *
