...
| 필드명       | 타입     | 설명                                                                                                                                        |
| --------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `sampled` | string | Zipkin의 [B3 Header](https://github.com/openzipkin/b3-propagation) 규약에 따른 필드로, 로그 추적 대상 여부를 나타냅니다.  <br>값이 `"1"`이면 추적 대상이며, `"0"`이면 제외됩니다. |
| `flags`   | string | 고급 디버깅 또는 트레이싱 강제 활성화를 위한 필드입니다.  <br>일반적으로 `"1"`은 디버깅 강제 활성화, `"0"`은 기본 동작을 의미하며, 선택적으로 사용됩니다.                                           |

> 참고: `meta`는 유연한 구조를 가지며, **필수 필드는 `sampled` 하나뿐**입니다.  
       서비스별로 필요한 값을 동적으로 추가할 수 있으며,  
	   시스템 간 연동 또는 모니터링 특성에 따라 확장 필드가 정의될 수 있습니다.
