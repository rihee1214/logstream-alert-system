...
- [ ] 2025-07-24 ~ 07-27
	- [ ] logging service의 메트릭 정보, 그리고 로그의 error, warn 로그 등을 볼 수 있는 모니터링 서비스 구축이 필요함.
	- [ ] 코드 스멜 발생중. LogCollector, LogValidator, LogPersistence 이 세개는 하나의 인터페이스를 구현하도록 하고, 파이프 라인으로 동작하게 만들어서 worker는 각각의 파이프 라인을 잘 모르게 하고, config가 그것들을 worker에 제공해서 worker는 그것이 뭔지도 모르고 그저 하나씩 실행하는 구조로 만들어야 한다.
		- [ ] worker.strategy=collect,validate,persistence 같은식으로 두기
		- [ ] enum에서 각각에 대한 이름과 실제 spec class를 묶어 넣기
		- [ ] properties의 순서에 따라서 config의 List에 spec객체 담기
		- [ ] enum 메서드에 `Map<String, String> setting`을 넣으면 spec 인스턴스 생성.
		- [ ] worker 객체가 사용하도록, spec에서 순서대로 인스턴스 가져와서 list로 return.
		- [ ] spec객체들에 대한 공통 상위 인터페이스 작성
		- [ ] spec객체들에 대한 정적 팩토리 메서드 제거.
	- [ ] implSpec annotation으로 상속에 대한 설명 적기
- [ ] 2025-08-01 ~ 03
	...
	- [ ] 기존에 `List<LogMessage>`로 두었던 LogProcessor의 출력과 입력을 LogProcessingContext를 인터페이스로 바꾼다. 그리고 pipeline안에 context패키지를 넣어서 그 구현체들을 넣도록 한다.
---
# 이미 작업한 목록
#done
