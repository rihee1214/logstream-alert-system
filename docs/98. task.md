...
- [ ] 2025-08-01 ~ 03
	...
	- [ ] kafka key에 서비스,호스트 이름, 트랜잭션 아이디 등을 넣어서 어디에서 문제가 발생하였는지 알 수 있는 단초를 제공해야한다. (단초만 제공한다. 애초에 근본적인 원인을 확인하는것은 불가능하다. producer가 잘못되어있기 때문에 로그 수집기 입장에서는 해당 서비스에 무슨 문제가 있는지 알 방도가 없고, 그 역할도 가질 수 없다.)
