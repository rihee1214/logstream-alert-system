...
- [ ] 2025-07-24 ~ 07-27
	- [ ] logging service의 메트릭 정보, 그리고 로그의 error, warn 로그 등을 볼 수 있는 모니터링 서비스 구성에 대한 고민이 필요함.
...
- [ ] 2025-08-06
	- [ ] logging-service 각 단계를 ProcessResult로 처리하도록 함. 모든 Exception에 대해서 catch하여 worker의 동작에 영향이 없도록 할 것.
...
- [ ] 2025-08-07
	- [ ] 테이블 생성 코드
		...
		```
- [ ] 2025-08-12
	- [ ] 현재 프로젝트는 로깅 서비스와 알림 서비스 두 가지로 이루어져 있음.
	- [ ] MVP 및 제대로 된 구성 을 위해 로깅 서비스 우선으로 모든 프로젝트의 초점을 맞출것.
	- [ ] 그다음 모든 피트니스 및 구성이 완료된 다음 알림서비스 작업 진행
	- [ ] 개발 테스트 환경 및 문서를 그에 맞출 준비 해야함
---
# 이미 작업한 목록
#done
