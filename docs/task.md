...
- [ ] 2025-07-17 (logging-service 작업 전략)
	...
	- [ ] 동작 전략 고민
		...
		- single thread로 동작하게 하되, 인스턴스를 여러개 실행시켜서 처리량을 따라갈 수 있도록 한다.
		- kafka에서 실패했다고 생각하는 시간을 초과하지 않는 선에서 모든 작업을 완수할 수 있도록 시간을 잡는다.
		- postgreSQL에서 subpartition으로 서비스를 지정하는 것은 현재 동작하지 않는다.
	- [ ] 문제점 고민
		- [ ] INSERT 실패시 문제가 있는 로그만을 어떻게 알아내서, 해당 요소만 제거한 후 재시도 할 수 있는지
		- [ ] 실패한 로그를 어떻게 저장할지
		- [ ] 셧다운 시그널로 graceful shutdown 전략 고민
		- [ ] 인스턴스가 많아지면, 서로가 메시지를 가져가려고 하기 때문에 모두가 조건을 만족하지 못하고 시간에 따른 commit만 가능하다는 점, 이전 로그가 나중에 insert될 수 있다는 점 고민 필요
---
# 이미 작업한 목록
#done
