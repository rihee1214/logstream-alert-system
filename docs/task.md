...
- [ ] 전체 목표 재 고려
	...
	- [ ] 프로젝트의 규모는 중-대규모의 구조를 가졌다는 것을 전제로 만들어짐
		...
		- [ ] 미래 확장성 트레이닝
	- [ ] 그러면서도 MVP는 놓치지 말기.
		- [ ] MVP로 각각 단일 인스턴스 고려( ex) prometheus -> alertmanager : 해당 조합은 중-대규모에 맞지 않지만 MVP로는 맞음)
- [ ] 2025-06-24 고려사항
	- [ ] alertmanager와 prometheus 연합에 대한 고민 필요(HA구성 관련)
	- [ ] S3 저장소 어떤거 사용할지에 대한 고려 필요
- [ ] 2025-06-25 고려사항
	- [ ] scyllaDB의 window에서 제약사항 확인 (제대로 된 세팅은 윈도우 이외의 환경에서만 가능)
	- [ ] scyllaDB는 작업 스케쥴링을 윈도우의 것을 사용하는게 아닌 자체 스케쥴러(seastar) 사용
	- [ ] scyllaDB는 저장소의 종류, 이외 여러가지 종류에 따라 세부 설정이 달라짐
- [ ] 2025-07-01 고려사항
	...
	- [ ] alert rule service에 대한 정책 고민
		- [ ] DB에 저장할 것인지
			- [ ] DB에 저장하면, 결국 정책의 변경으로 인한 슬라이딩 윈도우 변환도 문제고, 계속 DB를 보면서 활동하게 할 수도 없는 노릇임
			- [ ] 결국 구현 난이도와 설계 난이도는 이게 더 높을듯
		- [ ] 코드와 properties로 저장할 것인지
- [ ] 2025-07-10
	- [ ] ScyllaDB에 대한 의구심 제기
		- [ ] 정말 클라우드 환경에서 확장성과 유지보수성이 뛰어날 것인지
		      (운영적 요소까지 고려해야함)
		- [ ] 확장성을 추가할수는 없는지
		      (각 biz 프로젝트의 특화된 쿼리 실행으로 원하는 로그를 가져 올 수 있도록 해야하는데, scyllaDB는 그것이 많이 부족해보임)
- [ ] 2025-07-12
	- [ ] ScyllaDB는 ACID 트랜잭션을 제공하지 않기 때문에, insert 후 kafka 메시지 전송 자체가 원 트랜잭션으로 묶이지 않는다는 한계점이 있다.
	- [ ] 심지어 outbox transactional 패턴을 사용하기에도 애매한 것은 서로 다른 aprtitioning key를 사용하면 batch로 묶는 것 또한 비효율/비권장 된다는 점도 크다.
- [ ] 2025-07-17~18 (logging-service 작업 전략)
	- [ ] 거추장스럽고 무거운 Spring Boot 완전 제거
	- [ ] 동작 전략 고민
	...
	- [ ] 문제점 고민
		...
		- [x] 실패한 로그를 어떻게 저장할지
		- [ ] 셧다운 시그널로 graceful shutdown 전략 고민
	...
	- [ ] 내부 구조 고민
		- [ ] 멀티스레드로 동작하게 하고, 최대로 받을 로그의 양과 최대로 얼마동안 데이터를 수집할지 고민한다.
		- [ ] insert 전에 작업해야할 내용은 다음과 같다.
			- 로그 데이터를 DTO에 담기
			1. 로그 데이터에 대한 검증 진행 (필수 값 여부와 데이터 타입 검증)
			2. 로그 데이터 필드명 변경 (.으로 되어있는 요소를 \_로 변경)
			3. 기본 정보값을 제외한 요소는 extra 필드에 JSON 형식으로 보유
		- [ ] postgreSQL에서 나중에 들어온 데이터가 INSERT될 때 성능 이슈가 크게 없도록 PK값 고민
		- [ ] graceful shutdown 구현해야하는 요소
			1. kafka poll 후 DB insert 여부에 따른 consumer 처리
			2. 메시지 처리 중 예외 발생 후 종료
			3. 멀티 스레드 환경에서 일부 작업 중단
		- [ ] 결국 안정성을 높이기 위해 각 로그마다 id를 생성하도록(UUIDv7, ULID(멀티스레드 환경 우세)) 관련 로직을 provider에 추가 하고, postgreSQL에서 insert할때 `ON CONFLICT (log_id) DO NOTHING` 같은 식으로 처리하여, 다시 insert되지 않도록 한다.
- [ ] 2025-07-19 (문서 관리 고민 사항)
	- [ ] 현재 작성되어있는 모든 문서들을 old영역에 백업해두고, 제대로 관리할 수 있는 환경을 다시 만들어야함
		...
		- 결국 다음과 같은 작업이 필요하다고 판단됨
			- [ ] 결정을 내리기 전 제대로 확인할 수 있는 작업 제안서 항목 추가
			- [ ] 결정문서 모두 정리하고, 아키텍처 문서와 합치기
			- [ ] 개인 고민, 임시 문서, 플랜 문서 전부 정리하고 백로그 추가
	- [ ] 시스템을 이해하기 위해 필요한 모든 문서들을 모아둔 상위 폴더 필요
