<%*
const statusOptions = ["in-progress", "done", "canceled", "deferred"];
const statusLabels = {
  "in-progress": "🔁 적용 예정",
  "done": "✅ 적용 완료",
  "canceled": "❌ 결정 취소",
...
result += `- ${label}\n`;
	...
	- 작성자 : 이름
	- 담당자
	- 결정 일자 : YYYY-MM-DD (or ASAP)
	- 관련 문서 : `[관련 문서명](경로)`
result += `    - 참석자 : 이름1, 이름2\n`;
result += `    - 작성자 : 이름\n`;
result += `    - 담당자 : 이름\n`;
result += `    - 결정 일자 : YYYY-MM-DD\n`;
result += `    - 작업 일자 : YYYY-MM-DD\n`;
result += `    - 관련 문서 : [관련 문서명](경로)\n`;

result += `\n_(취소 및 보류 시 관련 문서 및 사유는 별도로 여기에 작성)_\n`;
