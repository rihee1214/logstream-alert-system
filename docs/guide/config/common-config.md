...
Spring Boot 기반 서비스의 설정 값들을 실제 주입 위치에 따라 다음 세 가지 파트로 구분하여 설명합니다:
...
- 설정 우선순위는 다음과 같습니다:
  ...
  2. 환경 변수 (`ENV_VAR`)
  3. application.properties
- ISO-8601 Duration 값은 문자열(`PT3S`)로 주입되며, 숫자만 입력하거나 포맷이 틀리면 예외 발생 가능성이 있습니다.
- 필수값 누락 시 애플리케이션이 기동되지 않거나 self-monitoring이 동작하지 않을 수 있습니다.